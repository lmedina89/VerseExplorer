...
For COMMIT, Explorer recomputes the parent state, observer state and reticle direction at the exact commit instant. Initial circular speed remains `v = sqrt(G(M_parent + m_asteroid)/r)`, with the parent inertial velocity included. The tangent is chosen prograde relative to the parent rotation axis. After insertion there is no enforced Kepler rail: the asteroid becomes a normal gravity source in the existing direct mutual Newtonian solver and velocity-Verlet integrator, so perturbations, collisions and escape remain possible.

The amber surface-sky ghost and projected orbit are visualization aids only. The ghost is deliberately enlarged so a physically tiny asteroid can be aimed on a phone display; this does not change the committed body's 1.0e12 kg mass, 3000 kg/m³ density or derived radius. LOW / MEDIUM / HIGH shells are curated insertion presets, not claims about operationally safe real spacecraft orbits. The massless reference **SURFACE SKY** observer intentionally cannot spawn bodies; SKY SPAWN requires the actual landed/on-foot state.

The earlier v0.1.0.6A flight LAB insertion path remains available as a secondary engineering/debug route, but the intended user-facing SKY SPAWN workflow is surface-first. Custom mass/radius/density, retrograde/free orbital elements, stars/black holes, binaries, rings/belts and arbitrary off-surface editors remain outside this bridge milestone.

# Universe Explorer v0.1.0.6A — Orbit Sandbox scientific boundary

The orbit sandbox does not introduce a second physics engine. PREVIEW is a nonphysical visualization only. For COMMIT, Explorer computes a two-body circular initial condition with `v = sqrt(G(M_parent + m_asteroid)/r)` in the parent's equatorial plane, adds the parent's current inertial velocity, and uses the parent's existing rotation axis to choose the prograde tangent. The state is recomputed at the exact commit instant rather than reusing a stale preview position/velocity.

After insertion there is no enforced Kepler rail or scripted orbit. The 1.0e12 kg basalt asteroid is a normal gravity source in the existing direct mutual Newtonian solver and velocity-Verlet integrator, so other bodies may perturb it and collisions/escape remain possible. A circular state is therefore an **initial condition**, not a promise that the orbit remains perfectly circular in the full N-body system.

The current radius is derived from the fixed 3000 kg/m³ density and mass; material/color are presentation metadata. LOW / MEDIUM / HIGH altitudes are curated insertion presets, not claims about operationally safe real spacecraft orbits. This milestone deliberately omits arbitrary orbital elements, custom mass/radius/density, retrograde insertion, surface-reticle placement, binary-system construction, stars/black holes and ring/belt generation. `SOL — MODIFIED` is provenance/UI state indicating that the reference initial system has been altered; it does not modify physical equations.
