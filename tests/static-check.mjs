...
const required = [
  'index.html','styles.css','src/main.js','src/app/app.js','src/core/constants.js','src/data/systemGenerator.js','src/data/generationProfiles.js','src/data/solSystem.js',
...
if (!html.includes('Universe Explorer v0.1.0.6A.3-FW2.1')) throw new Error('Explorer shell version is not v0.1.0.6A.3-FW2.1.');
if (!html.includes('<div class="brand">UNIVERSE EXPLORER <span>v0.1.0.6A.3-FW2.1</span></div>')) throw new Error('Visible top-left build badge is not Universe Explorer v0.1.0.6A.3-FW2.1.');
if (!html.includes('./src/main.js?v=ue0106a3fw21')) throw new Error('v0.1.0.6A.3-FW2.1 main module cache marker missing.');
...
if (!html.includes('./styles.css?v=ue0106a3') || !html.includes('./src/main.js?v=ue0106a3')) throw new Error('Build-version cache-busting tags missing.');
if (!html.includes('id="generationProfile"') || !html.includes('value="abyssal"') || !html.includes('value="sol"')) throw new Error('Universe generation profile selector missing.');
...
for (const id of ['sandboxChip','sandboxParent','sandboxAltitude','sandboxAltitudeValue','sandboxSpeedValue','sandboxPeriodValue','sandboxBodyValue','sandboxPreview','sandboxCommit','sandboxCancel','sandboxStatus']) if (!html.includes(`id=\"${id}\"`)) throw new Error(`Orbit sandbox control missing: ${id}`);
...
for (const id of ['cosmosToggle','cosmosPanel','phenomenonSelect','phenomenonReality','phenomenonScanDepth','mapToggle','mapPanel','systemMapCanvas','mapBodySelect','mapViewMode','mapSelectionFrameArrival','mapZoom','mapUnknownToggle','mapSelectAction','mapScanAction','mapTransitAction','mapCosmosAction','mapEventsAction','eventsPanel','eventsClose','eventObserverName','eventObserverModel','eventStartTime','eventHorizon','eventSearch','eventCancel','eventProgressFill','eventProgressText','eventStatus','eventResults','scanPhenomenon','observePhenomenon','orbitPhenomenon','landFlatWorld','nextPhenomenon','rendezvousPhenomenon','shipViewCosmos','compactObjectType','neutronStarMass','pulsarSpinPeriod','pulsarMagneticField','spawnNeutronStar','extremeObjectType','spawnExtremeObject','spaceWeatherActive','spaceWeatherNext','spaceWeatherStatus','triggerCme','autoWeatherToggle','overlayToggle','overlayPanel','overlayMaster','overlayLagrange','overlayHill','overlayRoche','overlayGravity','overlayOrbitPlane']) if (!html.includes(`id=\"${id}\"`)) throw new Error(`Cosmic exploration control missing: ${id}`);
...
if (!css.includes('html,body{margin:0;width:100%;height:100%;overflow:hidden;background:var(--bg);font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;color:var(--text);touch-action:manipulation}')) throw new Error('Global iOS double-tap zoom suppression missing.');
if (!css.includes('.app-shell{position:relative;width:100%;height:var(--app-height);max-height:var(--app-height);overflow:hidden;background:#000;touch-action:manipulation}')) throw new Error('Explorer shell double-tap zoom suppression missing.');
...
const flatWorldSurface = await readFile(new URL('../src/render/flatWorldSurface.js', import.meta.url), 'utf8');
const flatWorldObservation = await readFile(new URL('../src/surface/flatWorldObservation.js', import.meta.url), 'utf8');
for (const token of ['FlatWorldSurfaceVisual','createFlatWorldAnomalyVisual','updateFlatWorldAnomalyVisual','firmamentEdgeMaterial','observerPosition']) if (!flatWorldSurface.includes(token)) throw new Error(`FW2 Flat World surface token missing: ${token}`);
for (const token of ['createFlatWorldObservationRegion','flatWorldObservation','presentation-only-disc-top']) if (!flatWorldObservation.includes(token)) throw new Error(`FW2 Flat World observation-region token missing: ${token}`);
if (!renderer.includes("./flatWorldSurface.js?v=ue0106a3fw21")) throw new Error('FW2 Flat World surface renderer bridge missing.');
if (!main.includes("./app/app.js?v=ue0106a3")) throw new Error('Main-to-app cache-busting import missing.');
if (!app.includes("../render/threeRenderer.js?v=ue0106a3") || !app.includes("../experiments/orbitSandbox.js?v=ue0106a3")) throw new Error('Changed 0.1.0.6A.3 app cache-busting imports missing.');
if (!renderer.includes("./surfaceWorld.js?v=ue0106a3") || !renderer.includes("./celestialFactory.js?v=ue0106a2")) throw new Error('Changed 0.1.0.6A.3 renderer visual cache-busting imports missing.');
for (const inheritedImport of ["../core/astronomicalObserver.js?v=ue0105f","../ui/hud.js?v=ue0105f","../ui/systemMap.js?v=ue0105f","../surface/surfaceSkyObserver.js?v=ue0105f","../surface/surfaceGenerator.js?v=ue0105f","../surface/surfaceProfiles.js?v=ue0105f","../physics/frameOrbitInsertion.js?v=ue0105f"]) if (!app.includes(inheritedImport)) throw new Error(`Inherited 0.1.0.5F app dependency marker changed unexpectedly: ${inheritedImport}`);
if (!renderer.includes("./cockpitView.js?v=ue0105f")) throw new Error('Inherited cockpit renderer cache marker changed unexpectedly.');
