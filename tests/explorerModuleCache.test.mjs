...
test('Explorer 0.1.0.5A cache-busts every changed runtime module edge needed by Safari/GitHub Pages', () => {
  const index = read('index.html');
  const main = read('src/main.js');
  const app = read('src/app/app.js');
  const generator = read('src/data/systemGenerator.js');
  assert.match(index, /src\/main\.js\?v=155-ue0105a/);
  assert.match(main, /app\/app\.js\?v=155-ue0105a/);
